...
function CardList({ loading, setLoading }: Props) {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      setLoading(true);
      try {
        const fetchedCards = await getPaginatedAllCards(currentPage, pageSize, {
          ...filters,
    ...
    <div>
      <section className="mb-8 border-y border-white/10 py-5">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black text-white">Tune your search</h2>
          </div>
      ...
      {loading ? (
        ...
        <div className="rounded-[2rem] border border-amber-200/15 bg-white/[0.05] p-10 text-center">
          <h3 className="text-2xl font-black text-white">No cards found</h3>
          <p className="mt-3 text-slate-300">Try changing or clearing your filters.</p>
        </div>
      ) : (
